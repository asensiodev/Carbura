...
- [ ] 6.1 Select or install a full JDK containing `jpackage` without changing the Android toolchain unexpectedly
- [ ] 6.2 Generate a clean macOS DMG and inspect it for privileged credentials, tokens, and sensitive logs
...
- [ ] 6.4 Record Windows MSI, signing, Credential Manager, and installed-runtime validation as external until run on Windows
- [ ] 6.5 Run strict OpenSpec validation, full quality checks, all tests, Android assembly, Desktop package checks, and `git diff --check`
- [ ] 6.6 Review, commit, and push packaging and release-evidence changes
